	...
	switch(admincaster_screen)
		if(0)
			...
			dat+={"<HR><B>Feed Security functions:</B><BR>
				...
				<BR><A href='?src=\ref[src];ac_menu_censor_channel=1'>Mark Feed Channel with Weyland-Yutani D-Notice (disables and locks the channel.</A>
		...
		if(9)
			...
			if(src.admincaster_feed_channel.censored)
				dat+={"
					<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a Weyland-Yutani D-Notice.<BR>
		...
		if(10)
			dat+={"
				<B>Weyland-Yutani Feed Censorship Tool</B><BR>
		...
		if(11)
			dat+={"
				<B>Weyland-Yutani D-Notice Handler</B><HR>
